"""Seeded fixture for g-computation: a confounded cohort with a binary outcome.
...
The conditional odds ratio for treatment is exp(0.7) = 2.01 in every stratum; the MARGINAL risk
difference, risk ratio and odds ratio are population averages over L and Z, and the marginal odds ratio
is smaller than the conditional one (non-collapsibility), with no confounding involved. `truth()`
integrates the marginal quantities from the data-generating process.
